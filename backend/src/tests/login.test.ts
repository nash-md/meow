...
test.serial(`/login ${context.user.name} without password returns 400`, async (t) => {
  const res = await request(URL)
    .post('/public/login')
    .set('Content-Type', 'application/json')
    .send({
...
test.serial(`/cards without token returns 401`, async (t) => {
  const res = await request(URL).get('/api/cards').set('Content-Type', 'application/json');
...
test.serial(`/cards with an invalid signed JWT returns 401`, async (t) => {
  const token = createJwt(context.user._id, context.user.teamId, 86400);
  const res = await request(URL)
    .get('/api/cards')
    .set('Content-Type', 'application/json')
    .set('Token', token);
